    ...
    if ($(`#form${id}`).length === 0) {      
        ...
        $(`
            ...
            <div class="form-group">
                <label> ¿Tiene conectividad a internet la Institución?</label>
            ...
            <hr>
            <div class="form-group" id="type_conectividad" style="display:none">
              <div class="alert alert-danger" style="display: none" id="type_conectividad_error">
                Ha indicado que la institución cuenta con  conectividad a internet, debe seleccionar algún tipo de conexión 
              </div>
              <div class="checkbox">
                <label>
              ...
              <div class="checkbox">
                <label>
                  <input type="checkbox" id="salta_conectada" class="check_conectividad form-control"  name="type_conectividad[]" value="2">SALTA CONECTADA
                </label>
              </div>
              <hr>
              <div class="checkbox">
                <label>
                  <input type="checkbox" id="particular" class="check_conectividad  form-control"  name="type_conectividad[]" value="3">PARTICULAR
                </label>
              </div>            
            </div>
            <hr>
            <div class="form-group" id="field_rti">
                <label>Indique el número de RTI en ejercicio con los que cuenta la institución</label>
                <input type="text" name="rti" id="rti" class="form-control" value="0"  
                required pattern="[0-9]{1}" 
                title="Solo puede ingresar números entre 0 y 9">
            </div>
            <hr>
            <div class="form-group">
                <label>Indique si la institución tiene una PC de escritorio o notebook para fines administrativo</label>
                <select name="pc_escritorio" id="pc" class="form-control">                    
                    <option value="SI">SI</option>
                    <option value="NO" selected>NO</option>
                </select>   
            </div>
            <hr>
            <div class="form-group">
                <input  type="submit" value="Guardar" id="btn_guardar" class="form-control btn btn-danger">
            </div>
        </form>
        `).appendTo(`#p_body${id}`)
        ...
        $('#btn_guardar').click(function(){
          ...
          let option = $("#conectividad option:selected").val()
          
          if (option ==='SI') {
            let pnce = $('#pnce').prop('checked')
            let salta_conectada = $('#salta_conectada').prop('checked')
            let particular = $('#particular').prop('checked')
            //alert(pnce)
            if ((pnce===false &&  salta_conectada===false && particular===false)) {
              $('#type_conectividad_error').show()
              return false    
            }            
          }
        ...
        //consulta datos de tabla escuela (conectividad, pc_escritorio, rti)
        $.ajax({
          url: 'includes/mod_cen/clases/ajax/ajaxConectividad.php',
          type: 'POST',
          dataType: 'json',
          data: {escuela_id:id}
        })
        .done(function(lista) {
          for (let item of lista) {              
            ...
            if (item.conectividad ==="SI") {
                console.log('conectividad:'+item.conectividad);
                $("#conectividad option[value='SI']").prop("selected", true);

                $.ajax({
                  url: 'includes/mod_cen/clases/ajax/ajaxConectividad.php',
                  type: 'POST',
                  dataType: 'json',
                  data: {escuela_id_conectividad:id}
                })
                .done(function(lista) {
                  console.log(lista)
                  for (let item of lista) {
                    //console.log('Servico Id'+item.conectividad_servicio_id)
                    switch (item.conectividad_servicio_id) {
                      case '1':
                        $('#pnce').prop('checked', true);              
                        break;
                      case '2':
                        $('#salta_conectada').prop('checked', true);      
                         break;
                      case '3':
            ...
            } else if(item.conectividad ==="NO"){
              $("#conectividad option[value='NO']").prop("selected", true);
            }


            if (item.pc_escritorio ==="SI") {
              $("#pc option[value='SI']").prop("selected", true);
              console.log('Pc escritorio:'+item.pc_escritorio);
            } else if(item.pc_escritorio ==="NO"){
              $("#pc option[value='NO']").prop("selected", true);
              console.log('Pc escritorio:'+item.pc_escritorio);
            }
            

            console.log(item.rti)
            if(item.rti ===null){
              $('#rti').val('0')
            }else{
              $('#rti').val(item.rti)
        ...
        $('#conectividad').on( 'change', function(){
          if($(this).val()==='NO'){
            $('#type_conectividad').hide()
            $('.check_conectividad').prop('checked', false);

          }else if($(this).val()==='SI'){
            $('#type_conectividad').show()
          }

          //alert('cambio en tipo conectividad')
        })  
        $(`#p_body${id}`).show()
      }else{
        $(`#p_body${id}`).hide()
