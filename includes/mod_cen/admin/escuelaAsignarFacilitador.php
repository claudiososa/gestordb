...
if(($_POST))
	{
				...
				while ($fila = mysqli_fetch_object($resultado))
				{
					...
					$crearreferente=new Referente($fila->referenteIdFacilitador);
			  		...
			  		if($encontrado==0){
			  				...
			  				echo "<div class='divSimple' id='sel_".$fila->referenteIdFacilitador."'>";
			  			$ett= new Referente();

							$buscar_ett=$ett->buscarRef2("Facilitador");

							echo "<select disabled id='seleref_".$fila->referenteIdFacilitador."' name='referentes' >";
			  			...
			  			echo "<div class='divSimple' id='sel_".$fila->referenteIdFacilitador.$encontrado."'>";

			  			$ett= new Referente();
							$buscar_ett=$ett->buscarRef2('Facilitador');

			  			echo "<select  disabled id='seleref_".$fila->referenteIdFacilitador.$encontrado."' name='referentes' >";
			  			//echo	"<option value=0>Todos</option>";
			  			echo	"<option value='0001'>Sin Asignar</option>";
			  			while ($fila1 = mysqli_fetch_object($buscar_ett))
			  			{
			  				...
			  				$nombre=$persona->getNombre();
			  				$apellido=$persona->getApellido();
			  				if($fila1->referenteIdFacilitador==$_SESSION["referenteId"])
								echo	"<option value='".$fila1->referenteId."' selected >$apellido".",&nbsp;".$nombre."</option>";
								//echo	"<option value='$fila1->referenteId' >$apellido".",&nbsp;".$nombre."</option>";
								}
			  			...
			  			echo "</select></div>";
			  				...
			  				if($encontrado==0)
			  					echo "<div class='divSimple'  value='esc_".$fila->escuelaId."'  id='ref_".$fila->referenteIdFacilitador."'>"."<a href='index.php?mod=slat&men=referentes&id=2&personaId=".$r_personaId."&referenteId=".$fila->referenteId."'>".$apellidoPersona.", ".$nombrePersona.
					  			"</a></div>";
					  		else
					  			echo "<div class='divSimple' value='esc_".$fila->escuelaId."' id='ref_".$fila->referenteIdFacilitador.$encontrado."'>"."<a href='index.php?mod=slat&men=referentes&id=2&personaId=".$r_personaId."&referenteId=".$fila->referenteId."'>".$apellidoPersona.", ".$nombrePersona.
					  			"</a></div>";
					  		...
					  		if($encontrado==0)
					  			...
					  			echo "<input type='image' src='img/iconos/modificar_p.png'  height='22' width='22' value='".$fila->referenteIdFacilitador."' id='b_".$fila->referenteIdFacilitador."'/>
					  	  		<input type='image' src='img/iconos/guardar.png'  height='22' width='22' value='".$fila->referenteIdFacilitador."' id='g_".$fila->referenteIdFacilitador."'/>
					  	  	</div>";
					  	  	else
					  	  		echo "<input type='image' src='img/iconos/modificar_p.png'  height='22' width='22' value='".$fila->referenteIdFacilitador.$encontrado."' id='b_".$fila->referenteIdFacilitador.$encontrado."'/>
					  	  		<input type='image' src='img/iconos/guardar.png'  height='22' width='22' value='".$fila->referenteIdFacilitador.$encontrado."' id='g_".$fila->referenteIdFacilitador.$encontrado."'/>
					  	  		</div>";
			...
			$(document).ready(function(){
				...
				//alert(boton);
				 ...
				 $('[id^=g_]').click(function () {
					 ...
					 $.post("includes/mod_cen/clases/escuela.php", { facilitador:facilitador,referente_id: referente_id, escuela_id: escuela_id }, function(data){
