...
//buscar datos de escuela de acuerdo a tipo de Referente
//*****************************************************
  if (isset($_POST['tipoReferente'])) {
      $arrayPrincipal=array();
      $escuelas =  new EscuelaReferentes(null,null,$_POST['tipoReferente'],$_POST['referenteId']);
      $listaDeEscuelas = $escuelas->buscar2();
      Maestro::debbugPHP($_POST);
      while($datoEscuela = mysqli_fetch_object($listaDeEscuelas)){
        $item = array();
        $item=[
            'escuelaId' => $datoEscuela->escuelaId,
